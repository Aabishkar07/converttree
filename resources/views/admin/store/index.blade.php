...
@section('body')
    ...
    <div class="flex items-center">
        <div class="flex-1">
            <h2 class="w-full text-2xl font-bold text-secondary ">Our Store</h2>
        </div>
        <div class="text-right "> <a href="{{ route('admin.store.create') }} "
                    ...
                    <table class="min-w-full divide-y divide-gray-200">
                        ...
                        @foreach ($store as $key => $stores)
                            <tbody class="bg-white divide-y divide-gray-200 ">
                                <tr>
                                    <td class="px-5 py-3 ">
                                        <p class="text-gray-900 ">{{ $stores->title }}</p>
                                    </td>

                                    <td class="px-5 py-3 ">
                                        <p class="text-gray-900 ">{{ $stores->price }}</p>
                                    </td>
                                    ...
                                    <td class="w-48 px-5 py-3 text-sm">
                                        <p class="text-gray-900 ">
                                            {{ $stores->created_at->format('jS M Y') }}
                                        </p>
                                    </td>
                                    ...
                                    <td>
                                        <div class="flex items-center p-2">
                                            ...
                                            <a href=" {{ route('admin.store.edit', $stores->id) }}">
                                            ...
                                            <form method="POST" action="{{ route('admin.store.destroy', $stores->id) }}"
                                                id="delete-form-{{ $stores->id }}">
                                                @csrf
                                                @method('delete')
                                                <button type="button" onclick="deleteItem({{ $stores->id }})"
