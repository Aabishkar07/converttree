    ...
    <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div class="bg-white rounded-xl shadow-sm border border-gray-200">
            <div class="px-6 py-4 border-b border-gray-200">
                ...
                <p class="text-sm text-gray-500 mt-1">Update the details below to modify the user account</p>
            </div>

            <form action="{{ route('admin.usermanagement.update', $user->id) }}" method="POST" class="p-6 space-y-6">
                @csrf
                @method('PUT')
                        ...
                        @foreach ($roles as $key => $role)
                            <div class="relative">
                                <input type="radio" id="role_{{ $key }}" name="role" value="{{ $role->name }}"
                                    {{ old('role', $userRole ? $userRole->name : '') == $role->name ? 'checked' : '' }}
                                    class="sr-only peer">
                                <label for="role_{{ $key }}"
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        ...
                        <div class="relative">
                            <input type="radio" id="status_verified" name="status" value="VERIFIED"
                                {{ old('status', $user->status) == 'VERIFIED' ? 'checked' : '' }}
                                class="sr-only peer">
